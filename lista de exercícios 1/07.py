dia = int(input("Informe o dia do seu nascimento: "))
mes = int(input("Informa o mês do seu nascimento: "))

if dia >= 21 and mes == 1 or dia <=18 and mes == 2:
    print("Você é de Aquário!")
elif dia >= 19 and mes == 2 or dia <=20 and mes == 3:
    print("Você é de Peixes!")
elif dia >= 21 and mes == 3 or dia <=19 and mes == 4:
    print("Você é de Áries!")
elif dia >= 20 and mes == 4 or dia <=20 and mes == 5:
    print("Você é de Touro!")
elif dia >= 21 and mes == 5 or dia <=20 and mes == 6:
    print("Você é de Gêmeos!")
elif dia >= 21 and mes == 6 or dia <=22 and mes == 7:
    print("Você é de Câncer!")
elif dia >= 23 and mes == 7 or dia <=22 and mes == 8:
    print("Você é de Leão!")
elif dia >= 23 and mes == 8 or dia <=22 and mes == 9:
    print("Você é de Virgem!")
elif dia >= 23 and mes == 9 or dia <=22 and mes == 10:
    print("Parabéns, seu signo é o melho que existe, ou seja, LIBRAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA")
elif dia >= 23 and mes == 10 or dia <=21 and mes == 11:
    print("Você é de Escorpião!")
elif dia >= 22 and mes == 11 or dia <=21 and mes == 12:
    print("Você é de Sagitário!")
elif dia >= 22 and mes == 12 or dia <=20 and mes == 1:
    print("Você é de Capricórnio!")